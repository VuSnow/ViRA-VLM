...
class Train:
    def __init__(
        ...
        self.train_args = TrainingArguments(
            ...
            per_device_eval_batch_size=self.config.train_config.get("batch_size", 16),
            per_device_eval_batch_size=self.config.train_config.get("batch_size", 16),
            num_train_epochs=self.config.train_config.get("epochs", 3),
            learning_rate=self.config.train_config.get("learning_rate", 1e-5),
            ...
            gradient_accumulation_steps=self.config.train_config.get("gradient_accumulation_steps", 2),
            bf16=True,
            disable_tqdm=False,
            dataloader_num_workers=self.config.train_config.get("dataloader_num_workers", 4),
            optim="adamw_torch",
        )
